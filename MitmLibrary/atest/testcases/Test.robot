...
Do a post
    ...
    &{headers}  Create Dictionary  content=json
    Add Custom Response  alias=number_post  url=test_post  overwrite_headers=${headers}  overwrite_body=<number_size>not_found</number_size>  status_code=${404}
    Log Custom Response Items
    # Sleep  5s
    ${response}  POST on session  alias=proxy  url=test_post/1  expected_status=404
